...
__global__ void word_count(char *A, char *B, int *I, unsigned int *d_s, int spaces)
{
...
int main()
{
	...
	int *d_a, *d_b, *d_i;
	unsigned int *search = 0, *d_s, *result;
	...
	word_count<<<ceil(spaces/256.0), 256>>>(d_a, d_i, d_s, spaces);

	cudaMemcpy(result, d_s, sizeof(unsigned int), cudaMemcpyDeviceToHost);

	printf("String : \n%s\n",A);
	printf("Total occurances of NEW :%u\n",result);
